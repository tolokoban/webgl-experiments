...
// Coordonnée U pour la texture.
varying float varU;
// Profondeur. Utilisée pour assombrir les diamants lointains.
varying float varZ;
...
void main() {
  ...
  vec4 color = texture2D( uniTexture,
                          vec2( varU + gl_PointCoord.x * 0.0625,
                                gl_PointCoord.y ) );
