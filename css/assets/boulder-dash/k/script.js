...
WebGL.fetchAssets({
  ...
  function anim( time ) {
    ...
    if( env.nextSynchro < 0 ) {
      ...
      if( !heroIsDead ) processHero( heroPainter, env );
      diamPainter.process();
      boulderPainter.process();
      groundPainter.hitCell( heroPainter.Y + heroPainter.Vy, heroPainter.X + heroPainter.Vx );

      if( !heroIsDead && cave.get( heroPainter.Y, heroPainter.X ) !== 'E' ) {
        heroIsDead = true;
        console.warn("Hero is dead!");
      }
    }
    //#(synchro)
    ...
    env.y = Math.min(
      ...
      env.cave.rows - 1 - h * env.w * .5 / 64
    );
    //#(camera)
    
    // On affiche tout.
    env.z = 0.5;
    if( !heroIsDead ) heroPainter.draw( env );
...
function processHero( heroPainter, env ) {
  env.cave.set( heroPainter.Y, heroPainter.X, ' ' );
  heroPainter.X += heroPainter.Vx;
  heroPainter.Y += heroPainter.Vy;
  env.cave.set( heroPainter.Y, heroPainter.X, 'E' );
