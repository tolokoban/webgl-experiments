...
window.HeroPainter = function() {
  ...
  var WP = function( env ) {
    ...
    addProps( this, {
      X: {
        get: function() { return this._vertexArray[0] - .5; },
        set: function( v ) { this._vertexArray[0] = v + .5; }
      },
      Y: {
        get: function() { return this._vertexArray[1] - .5; },
        set: function( v ) { this._vertexArray[1] = v + .5; }
      },
      Vx: {
    ...
    for( row = 0; row < rows; row++ ) {
      for( col = 0; col < cols; col++ ) {
        if( cave.get( row, col ) === 'E' ) {
          vertexArray.push( col + .5, row + .5, 0, 0, 0 );
          break;
        }
      }
    }
