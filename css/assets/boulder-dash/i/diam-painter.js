window.DiamPainter = function() {
  ...
  WP.prototype.draw = function( env ) {
    ...
    prg.$texture = 0;
    // Attributs.
    prg.bindAttribs( this._buffVert, "attX", "attY", "attIndex" );
    // Dessin.
  ...
  function getVertexArray( cave ) {
    ...
    for( row = 0; row < rows; row++ ) {
      for( col = 0; col < cols; col++ ) {
        if( cave.get( row, col ) === 'd' ) {
          var diamIndex = Math.floor( 16 * Math.random() );
          vertexArray.push( col + .5, row + .5, diamIndex * 0.25 );
        }
      }
    }
