...
/** @module tfw.view */
require('tfw.view', function (require, module, exports) {
  var _ = function () {
    var D = {
      "en": {},
      "fr": {}
  ...
  function defineAttribFocus(elem) {
    ...
    PM(this).on("focus", function (v) {
      if (v) elem.focus();else elem.blur();
    });
    ...
    ["textContent", "textcontent"].forEach(function (name) {
      PM(this).create(name, {
        ...
        set: function set(v) {
          ...
          if (v.substr(0, 6) === '<html>') elem.innerHTML = v.substr(6);else elem.textContent = v;
        }
      });
    }, this);
  }
  ...
  function ensureCodeBehind(code_behind) {
    if (typeof code_behind === 'undefined') throw "Missing mandatory global variable CODE_BEHIND!";
    var i, funcName;

    for (i = 1; i < arguments.length; i++) {
      funcName = arguments[i];
      if (typeof code_behind[funcName] !== 'function') throw "Expected CODE_BEHIND." + funcName + " to be a function!";
    }
  }

  ;
  var GESTURES = ["tap", "doubletap", "press", "pan", "panstart", "panmove", "panup", "pandown", "panleft", "panright", "panend", "pancancel", "swipe", "swipeleft", "swipteright", "swipetop", "swipebottom", "pinch", "pinchin", "pinchout", "pinchstart", "pinchmove", "pinchend", "pinchcancel", "rotate", "rotatestart", "rotatemove", "rotateend", "rotatecancel"];
  /**
