...
/** @module webgl.fillpoly */
require('webgl.fillpoly', function (require, module, exports) {
  var _ = function () {
    var D = {
      "en": {},
      "fr": {}
  ...
  function vertexArrayToAttribs(vectors3) {
    try {
      var output = [];
      if (vectors3.length < 3) throw "[webgl.fillpoly.vertexArrayToAttribs] A polyline must have at least 3 vertices!";
      ...
      vectors3.forEach(function (vector3) {
        ...
        var y = vector3[1];
        var z = vector3[2];
        output.push(xI * x + yI * y + zI * z, xJ * x + yJ * y + zJ * z);
      });
  ...
  var Polyline = function Polyline(vertices, attributesCount) {
    var points = [];
  ...
  Polyline.prototype.isCandidate = function (orientation) {
    ...
    if (crossprod < 0 && orientation < 0) return false; // On vérifie maintenant qu'aucun point n'est à l'intérieur du triangle.
