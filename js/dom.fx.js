...
/** @module dom.fx */
require('dom.fx', function (require, module, exports) {
  var _ = function () {
    var D = {
      "en": {}
    },
  ...
  var EMPTY_FUNC = function EMPTY_FUNC() {// Do nothing.
  }; // Unique ID.
  ...
  function Fx(name) {
    ...
    this._session = {}; // A name can be defined for debug purpose, but it is not used right now.
    ...
    this._name = name; // Sequence of  tasks. A task  is a function with  two parameters:
    ...
    this._tasks = []; // Index of the current task.
  ...
  Fx.prototype.start = function (_onEnd) {
    var onEnd = typeof _onEnd !== "undefined" ? _onEnd : EMPTY_FUNC;
  ...
  function next(session) {
    ...
    if (this._debug) {
      console.info("[dom.fx] tsk[".concat(this._index - 1, "]: "), tsk.label, "(".concat(Date.now() - this._startTime, " ms)"), tsk.args, session);
    }
  ...
  Fx.prototype.exec = function () {
    ...
    this.addTask(function (next, session) {
      args.forEach(function (arg) {
        try {
          if (typeof arg === 'function') arg(session);else console.log('[dom.fx]', arg);
        } catch (ex) {
          console.error(ex);
        }
      });
  ...
  ['css', 'addClass', 'removeClass', 'toggleClass', 'detach', 'saveStyle', 'restoreStyle', 'add', 'removeAtt', 'replace'].forEach(function (methodname) {
    var slot = $[methodname];
  ...
  module.exports.Fullscreen = function (opts) {
    ...
    var voidFunc = function voidFunc() {};
