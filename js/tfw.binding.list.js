...
require('tfw.binding.list', function (require, module, exports) {
  var _ = function () {
    var D = {
      "en": {},
      "fr": {}
  ...
  */
  ...
  var List = function List(arg) {
    ...
    readOnly(this, "_links", []); // An object  with the  attribute `isContentChangeAware  === true`
  ...
  List.prototype.fire = function (functionName, args, wave) {
    var id = this[ID];
    if (!Array.isArray(wave)) wave = [id];else if (wave.indexOf(id) === -1) wave.push(id); // Listeners for content change.

    this._listeners.fire(functionName, args, wave); // Two Lists are linked if and only if they share the same array.
  ...
  ["push", "pop", "shift", "unshift", "splice", "reverse"].forEach(function (funcName) {
  ...
  ["slice", "forEach", "filter", "map", "reduce", "indexOf", "lastIndexOf"].forEach(function (funcName) {
    List.prototype[funcName] = function () {
