...
/** @module dom.theme */
require('dom.theme', function (require, module, exports) {
  var _ = function () {
    var D = {
      "en": {},
      "fr": {}
    ...
    for (var depth = 1; depth <= 10; depth++) {
      THEME_COLOR_NAMES.forEach(function (colorName) {
        ...
        for (var position = 1; position <= depth; position++) {
          ...
          codeCSS += "body.dom-theme-" + themeName + " " + piecesBG.join(" > ") + " { color: " + style['fg' + colorName] + " }\n";
  ...
  function codeBackground(themeName, style) {
    ...
    THEME_COLOR_NAMES.forEach(function (colorName) {
      ...
      codeCSS += "body.dom-theme-" + themeName + " .thm-svg-fill" + colorName + " { fill: " + style['bg' + colorName] + " }\n";
  ...
  function codeElevation(themeName, style) {
    ...
    var elevationColor = luminance < .6 ? addAlpha(style.white, Math.ceil(10 * luminance)) : addAlpha(style.black, '6');
    var codeCSS = '';
