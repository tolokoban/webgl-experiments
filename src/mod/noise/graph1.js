...
function vec(painter, x, y, text, position, degree) {
    const ang = degree * Math.PI / 180,
          vx = SIDE * Math.cos( ang ),
          vy = SIDE * Math.sin( ang );
    painter
        .setColor( painter.ORANGE )
        .drawVector( x, y, x + vx, y + vy )
        .setColor( "#fff" )
        .dot( x, y, text, position );
}
...
module.exports = require("draw")(function() {
    try {
        ...
        this
            ...
            .fillRect( -1, -1, +1, +1 )
            .setColor( "#CEF" )
            .fillRect( -SIDE, -1, SIDE, 1 )
            .fillRect( -1, -SIDE, 1, SIDE )
            .setColor( this.BLUE )
            .fillRect( -SIDE, -SIDE, SIDE, SIDE )
            .setColor( "#9CF" )
            .setLine( 1 )
            .drawVector( -SIDE, -SIDE, x, y )
            .drawVector( -SIDE, +SIDE, x, y )
            .drawVector( +SIDE, -SIDE, x, y )
            .drawVector( +SIDE, +SIDE, x, y )
            .setColor( "#000" )
            .drawLine( -SIDE, y, SIDE, y )
            .dot( -SIDE, y, "Vab", "R" )
            .dot( +SIDE, y, "Vcd", "L" )
            .setColor( "#fff" )
            .dot( x, y, "F", "BL" )
