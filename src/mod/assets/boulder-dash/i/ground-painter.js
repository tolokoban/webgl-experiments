window.GroundPainter = function() {
  ...
  //#(hitCell)
  /**
   * Quand le héro se déplace, il appelle cette  méthode avec sa position en argument. Si la cellule
   * contient des feuilles, on modifie les vertexArray et on le renvoie dans la carte graphique.
   */
  WP.prototype.hitCell = function( row, col ) {
    ...
    // supprimer.
    console.info("[ground-painter] row, col, idx, pointer, this._count=", row, col, idx, pointer, this._count);
    this._count--;
    var ptrLast = this._count * 2;
    // Evidemment, si celui qu'on veut supprimer se trouve aussi être le dernier de la liste, il n'y
